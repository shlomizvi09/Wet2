...
StatusType GetArtistBestSong(void *DS, int artistID, int *songId) {
  ...
  MusicManagerResult tmpResult;
  try {
    tmpResult = ds->GetArtistBestSong(artistID, songId);
  } catch (std::bad_alloc &e) {
    return ALLOCATION_ERROR;
  }
  if (tmpResult == MM_NOT_EXISTS || tmpResult == MM_FAIL)
...
StatusType GetRecommendedSongInPlace(void *DS, int rank, int *artistId, int *songId) {
  if (DS == nullptr || artistId == nullptr || songId == nullptr)
    return INVALID_INPUT;
  MusicManager *ds = (MusicManager *) DS;
  MusicManagerResult tmpResult;
  try {
    tmpResult = ds->GetRecommendedSongInPlace(rank, artistId, songId);
  } catch (std::bad_alloc &e) {
    return ALLOCATION_ERROR;
  }
